...
export const PreferenceInput: React.FC<PreferenceInputProps> = ({ onGenerate }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Adapt to backend response structure (which might return "tracks" lowercase)
      const tracks = response.Tracks || response.tracks;

      // Validate response structure
      if (!response || !tracks || !Array.isArray(tracks)) {
      ...
      const playlist = {
        ...
        Tracks: tracks,
        totalDuration: calculateDuration(tracks.length),
              ...
              <div className="flex flex-wrap gap-2">
                {genres.map(genre => (
                  <Badge
                    ...
                    className={`cursor-pointer transition-all ${preferences.genres.includes(genre)
              ...
              <div className="flex flex-wrap gap-2">
                {moods.map(mood => (
                  <Badge
                    ...
                    className={`cursor-pointer transition-all ${preferences.moods.includes(mood)
              ...
              <div className="flex flex-wrap gap-2">
                {decades.map(decade => (
                  <Badge
                    ...
                    className={`cursor-pointer transition-all ${preferences.decades.includes(decade)
